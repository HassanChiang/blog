...
date: 2020-05-20
description:
{: id="20201220214147-2pkxrfd"}

tags:
{: id="20201220214147-08ba8p9"}

- {: id="20201220214147-4fdv2aw"}Java
- {: id="20201220214147-5q0wubd"}Spring
- {: id="20201220214147-xnndiz8"}Spring Core
- {: id="20201220214147-43q5ut1"}源码分析
{: id="20201220214147-livxx9c"}

nav:
{: id="20201220214147-x1vfb7n"}

- {: id="20201220214147-gho9z6s"}Spring
{: id="20201220214147-fuqftoi"}

categories:
{: id="20201220214147-32xrj5h"}

- {: id="20201220214147-6j6re01"}Spring Core
{: id="20201220214147-1226vwv"}

image: images/java/spring-framework.png
{: id="20201220214147-h6fw2zj"}
...
{: id="20201220214147-r6c3ae4"}

希望通过本文可以让读者不惧怕阅读 Spring 源码，也希望大家能反馈表述错误或不合理的地方。
{: id="20201220214147-iqayzs8"}

## 引言
{: id="20201220214147-zllbcmx"}

先看下最基本的启动 Spring 容器的例子：
{: id="20201220214147-52vkx62"}
...
```
{: id="20201220214147-rivc0eo"}

> spring-context 会自动将 spring-core、spring-beans、spring-aop、spring-expression 这几个基础 jar 包带进来。
> {: id="20201220214147-guk4q0e"}
{: id="20201220214147-efowx0a"}
...
```
{: id="20201220214147-a3axf2f"}

定义接口实现类：
{: id="20201220214147-u7yd7qh"}
...
```
{: id="20201220214147-nxp1cl2"}

这样，我们就可以跑起来了：
{: id="20201220214147-bim8yl8"}
...
{: id="20201220214147-urxqiy4"}

## BeanFactory 简介
{: id="20201220214147-97ul8z2"}

BeanFactory，从名字上也很好理解，生产 bean 的工厂，它负责生产和管理各个 bean 实例。
{: id="20201220214147-dya8bte"}

初学者可别以为我之前说那么多和 BeanFactory 无关，前面说的 ApplicationContext 其实就是一个 BeanFactory。我们来看下和 BeanFactory 接口相关的主要的继承结构：
{: id="20201220214147-cntajou"}

![1](./2020-11-20_SpringIOC源码分析/2.png)
{: id="20201220214147-2pc8324"}

我想，大家看完这个图以后，可能就不是很开心了。ApplicationContext 往下的继承结构前面一张图说过了，这里就不重复了。这张图呢，背下来肯定是不需要的，有几个重点和大家说明下就好。
{: id="20201220214147-oio5yp0"}

1. {: id="20201220214147-0u7fl09"}ApplicationContext 继承了 ListableBeanFactory，这个 Listable 的意思就是，通过这个接口，我们可以获取多个 Bean，大家看源码会发现，最顶层 BeanFactory 接口的方法都是获取单个 Bean 的。
2. {: id="20201220214147-gfe9h4t"}ApplicationContext 继承了 HierarchicalBeanFactory，Hierarchical 单词本身已经能说明问题了，也就是说我们可以在应用中起多个 BeanFactory，然后可以将各个 BeanFactory 设置为父子关系。
3. {: id="20201220214147-offwbnb"}AutowireCapableBeanFactory 这个名字中的 Autowire 大家都非常熟悉，它就是用来自动装配 Bean 用的，但是仔细看上图，ApplicationContext 并没有继承它，不过不用担心，不使用继承，不代表不可以使用组合，如果你看到 ApplicationContext 接口定义中的最后一个方法 getAutowireCapableBeanFactory() 就知道了。
4. {: id="20201220214147-rzjcvi9"}ConfigurableListableBeanFactory 也是一个特殊的接口，看图，特殊之处在于它继承了第二层所有的三个接口，而 ApplicationContext 没有。这点之后会用到。
5. {: id="20201220214147-o6d4efe"}请先不用花时间在其他的接口和类上，先理解我说的这几点就可以了。
{: id="20201220214147-q2ikfvv"}
...
{: id="20201220214147-deja7p4"}

## 启动过程分析
{: id="20201220214147-v1f12sr"}

下面将会是冗长的代码分析，记住，一定要自己打开源码来看，不然纯看是很累的。
{: id="20201220214147-8yohlhq"}

第一步，我们肯定要从 ClassPathXmlApplicationContext 的构造方法说起。
{: id="20201220214147-wyq9cz8"}
...
```
{: id="20201220214147-fu97l87"}

下面，我们开始一步步来肢解这个 refresh() 方法。
{: id="20201220214147-tmezes1"}

### 创建 Bean 容器前的准备工作
{: id="20201220214147-0lhag08"}

这个比较简单，直接看代码中的几个注释即可。
{: id="20201220214147-qzmueli"}
...
```
{: id="20201220214147-4bvm3k6"}

// AbstractRefreshableApplicationContext.java 120
{: id="20201220214147-gf3lkez"}
...
> {: id="20201220214147-bhgicq2"}
{: id="20201220214147-g91zhmq"}
...
{: id="20201220214147-impytlr"}

![1](./2020-11-20_SpringIOC源码分析/3.png)
{: id="20201220214147-poqii45"}
...
>       default-autowire="byName">
> ```
> {: id="20201220214147-lecqkxx"}
{: id="20201220214147-f08pvnw"}

回过神来，看看处理 default 标签的方法：
{: id="20201220214147-7m22v95"}
...
{: id="20201220214147-z7uq19k"}

简单地说就是像下面这样子：
{: id="20201220214147-9rrrrqu"}
...
```
{: id="20201220214147-caqp04x"}

然后，我们再看看怎么根据配置创建 BeanDefinition 实例的：
{: id="20201220214147-y5tpkvc"}
...
```
{: id="20201220214147-bjvfwqv"}

到这里，我们已经完成了根据 `<bean />` 配置创建了一个 BeanDefinitionHolder 实例。注意，是一个。
{: id="20201220214147-4pgyptj"}

我们回到解析 `<bean />` 的入口方法:
{: id="20201220214147-s45s6op"}
...
...
```
{: id="20201220214147-ncc60bu"}

然后我们准备注册这个 BeanDefinition，最后，把这个注册事件发送出去。
{: id="20201220214147-cc8nalx"}

下面，我们开始说说注册 Bean 吧。
{: id="20201220214147-6eflstq"}

注册 Bean
{: id="20201220214147-jlo3294"}

// BeanDefinitionReaderUtils 143
{: id="20201220214147-b9z0au4"}
...
```
{: id="20201220214147-acsf99i"}

别名注册的放一边，毕竟它很简单，我们看看怎么注册 Bean。
{: id="20201220214147-yxbtjht"}

// DefaultListableBeanFactory 793
{: id="20201220214147-5qyqfl5"}
...
```
{: id="20201220214147-3j7fh3u"}

从上面最后一行往里看，我们就又回到 DefaultListableBeanFactory 这个类了，这个类大家应该都不陌生了吧。
{: id="20201220214147-gbahv01"}

preInstantiateSingletons
{: id="20201220214147-kywvejs"}

// DefaultListableBeanFactory 728
{: id="20201220214147-zkrr7nh"}
...
```
{: id="20201220214147-a11mptr"}

创建 Bean
{: id="20201220214147-0zonj06"}

我们继续往里看 doCreateBean 这个方法：
{: id="20201220214147-lv0gt99"}
...
```
{: id="20201220214147-bw97nce"}

挑个简单的无参构造函数构造实例来看看：
{: id="20201220214147-7za7ve4"}
...
```
{: id="20201220214147-c2j5k9h"}

我们可以看到，关键的地方在于：
{: id="20201220214147-ooutatk"}

```
beanInstance = getInstantiationStrategy().instantiate(mbd, beanName, parent);
```
{: id="20201220214147-46z17o0"}

这里会进行实际的实例化过程，我们进去看看:
{: id="20201220214147-0n8kq6x"}

// SimpleInstantiationStrategy 59
{: id="20201220214147-mo01a3v"}
...
```
{: id="20201220214147-pjvwqhh"}

initializeBean
{: id="20201220214147-7bdovhz"}

属性注入完成后，这一步其实就是处理各种回调了，这块代码比较简单。
{: id="20201220214147-i9y2416"}
...
```
{: id="20201220214147-8o2tw3t"}

应该不必做过多解释了吧，看每个文件第一行的 profile=""。
{: id="20201220214147-st001p3"}

当然，我们也可以在一个配置文件中使用：
{: id="20201220214147-5wj2dko"}
...
```
{: id="20201220214147-klqtq62"}

理解起来也很简单吧。
{: id="20201220214147-ae9umw4"}

接下来的问题是，怎么使用特定的 profile 呢？Spring 在启动的过程中，会去寻找 “spring.profiles.active” 的属性值，根据这个属性值来的。那怎么配置这个值呢？
{: id="20201220214147-u6g99bp"}

Spring 会在这几个地方寻找 spring.profiles.active 的属性值：操作系统环境变量、JVM 系统变量、web.xml 中定义的参数、JNDI。
{: id="20201220214147-p3zy87k"}

最简单的方式莫过于在程序启动的时候指定：
{: id="20201220214147-8xf4w8p"}

```
-Dspring.profiles.active="profile1,profile2"
```
{: id="20201220214147-cl0t3h6"}

> profile 可以激活多个
> {: id="20201220214147-8l4no51"}
{: id="20201220214147-opaxiak"}

当然，我们也可以通过代码的形式从 Environment 中设置 profile：
{: id="20201220214147-v55f5zv"}
...
```
{: id="20201220214147-tomrf11"}

实例工厂：
{: id="20201220214147-p6jjxyn"}
...
```
{: id="20201220214147-mtcu0av"}

### FactoryBean
{: id="20201220214147-446n8e3"}

FactoryBean 适用于 Bean 的创建过程比较复杂的场景，比如数据库连接池的创建。
{: id="20201220214147-8s8yl21"}
...
```
{: id="20201220214147-mgryg39"}

我们看看装配的时候是怎么配置的：
{: id="20201220214147-m07ydk6"}
...
```
{: id="20201220214147-yrrz6tv"}

这个时候，其实我们的思路也很简单，把 MyCarFactoryBean 看成是一个简单的 Bean 就可以了，不必理会什么 FactoryBean，它是不是 FactoryBean 和我们没关系。
{: id="20201220214147-ypgzhq1"}

### 初始化 Bean 的回调
{: id="20201220214147-rxjeivl"}

有以下四种方案：
{: id="20201220214147-4atg3a1"}
...
```
{: id="20201220214147-86pjhyv"}

### 销毁 Bean 的回调
{: id="20201220214147-ta5e0lk"}
...
```
{: id="20201220214147-w98vwdt"}

### ConversionService
{: id="20201220214147-5ttqqyt"}

既然文中说到了这个，顺便提一下好了。
{: id="20201220214147-i9iwh9x"}

最有用的场景就是，它用来将前端传过来的参数和后端的 controller 方法上的参数进行绑定的时候用。
{: id="20201220214147-p88qz05"}
...
```
{: id="20201220214147-yc9s39c"}

ConversionService 接口很简单，所以要自定义一个 convert 的话也很简单。
{: id="20201220214147-jxwy9sc"}

下面再说一个实现这种转换很简单的方式，那就是实现 Converter 接口。
{: id="20201220214147-ohsht5q"}

来看一个很简单的例子，这样比什么都管用。
{: id="20201220214147-8vho7rx"}
...
```
{: id="20201220214147-p65pzl8"}

xml 配置 `<lookup-method />`：
{: id="20201220214147-te3avcy"}
...
```
{: id="20201220214147-fde2p7i"}

> 注意，既然用了注解，要配置注解扫描：`<context:component-scan base-package="com.javadoop" />`
> {: id="20201220214147-nqryvvi"}
{: id="20201220214147-we7va0x"}

甚至，我们可以像下面这样：
{: id="20201220214147-thseut1"}
...
```
{: id="20201220214147-29pk4yk"}

> 上面的返回值用了 MyCommand，当然，如果 Command 只有一个实现类，那返回值也可以写 Command。
> {: id="20201220214147-g0rwkmu"}
{: id="20201220214147-rpgh97h"}

replaced-method
{: id="20201220214147-xe62w54"}

记住它的功能，就是替换掉 bean 中的一些方法。
{: id="20201220214147-1uiuo43"}
...
```
{: id="20201220214147-xdvxpzg"}

方法覆写，注意要实现 MethodReplacer 接口：
{: id="20201220214147-p6igopd"}
...
```
{: id="20201220214147-u9cngna"}

配置也很简单：
{: id="20201220214147-hjyvvag"}
...
```
{: id="20201220214147-g8cr8mf"}

> arg-type 明显不是必须的，除非存在方法重载，这样必须通过参数类型列表来判断这里要覆盖哪个方法。
> {: id="20201220214147-48l121l"}
{: id="20201220214147-43uin62"}

### BeanPostProcessor
{: id="20201220214147-naoc4zc"}

应该说 BeanPostProcessor 概念在 Spring 中也是比较重要的。我们看下接口定义：
{: id="20201220214147-nekhyv6"}
...
{: id="20201220214147-sxsjbah"}

## 总结
{: id="20201220214147-xckfdyd"}

按理说，总结应该写在附录前面，我就不讲究了。
{: id="20201220214147-0l3d45t"}

在花了那么多时间后，这篇文章终于算是基本写完了，大家在惊叹 Spring 给我们做了那么多的事的时候，应该透过现象看本质，去理解 Spring 写得好的地方，去理解它的设计思想。
{: id="20201220214147-aa5ouve"}

本文的缺陷在于对 Spring 预初始化 singleton beans 的过程分析不够，主要是代码量真的比较大，分支旁路众多。同时，虽然附录条目不少，但是庞大的 Spring 真的引出了很多的概念，希望日后有精力可以慢慢补充一些。
{: id="20201220214147-9odcies"}

（全文完）
{: id="20201220214147-kpsvkdo"}

转载自： https://juejin.cn/post/6844903694039793672
{: id="20201220214147-d5ywt9b"}


{: id="20201220214147-7ziqfoy" type="doc"}
